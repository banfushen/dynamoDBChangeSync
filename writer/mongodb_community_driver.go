...
func (mcw *MongoCommunityWriter) WriteBulk(input []interface{}, index []interface{}) error {
	if len(input) == 0 {
		return nil
	}
	...
	for i := range models {
		models[i] = &mongo.ReplaceOneModel{
			Filter:      index[i],
			Replacement: input[i],
			Upsert:      &[]bool{true}[0]}
	...
	if err != nil {
		if strings.Contains(err.Error(), "duplicate key error") {
			...
			for i, ele := range input {
				...
				index := make(bson.M, len(mcw.primaryIndexes))
...
func (mcw *MongoCommunityWriter) FetchCollection(changeC chan bson.M) error {
	collection := mcw.conn.Client.Database(mcw.ns.Database).Collection(mcw.ns.Collection)
	...
	if err != nil {
		LOG.Error("FetchCollection Failed to execute find operation: %v", err)
		return err
	}
	defer cursor.Close(ctx) // 确保在函数结束时关闭cursor

	// 遍历查询结果
	for cursor.Next(ctx) {
		var result bson.M
		if err = cursor.Decode(&result); err != nil {
			LOG.Error("FetchCollection Failed to decode document: %+v err: %+v", result, err)
			return err
		}

		LOG.Info("FetchCollection result %+v", result)

		select {
		case changeC <- result:
		default:
			LOG.Error("FetchCollection result in chan fail, len changeC %d", len(changeC))
		}
	}

	// 检查查询过程中是否发生错误
	if err = cursor.Err(); err != nil {
		LOG.Error("Cursor encountered an error: %v", err)
		return err
	}
	return nil
}
...
func (mcw *MongoCommunityWriter) createPrimaryIndex(primaryIndexes []*dynamodb.KeySchemaElement, parseMap map[string]string) error {
	...
	if conf.Options.TargetMongoDBType == utils.TargetMongoDBTypeSharding {
		res := mcw.conn.Client.Database("admin").RunCommand(nil, bson.D{
		...
		res = mcw.conn.Client.Database("admin").RunCommand(nil, bson.D{
			...
			{"key", bson.M{primaryKeyWithType: "hashed"}},
			{"options", bson.M{"numInitialChunks": NumInitialChunks}},
		})
		if err := res.Err(); err != nil {
...
func (mcw *MongoCommunityWriter) createSingleIndex(primaryIndexes []*dynamodb.KeySchemaElement, parseMap map[string]string,
	...
	if len(indexList) >= 2 {
		...
		for _, ele := range indexList {
			keysMap = append(keysMap, bson.E{ele, 1})
		}
